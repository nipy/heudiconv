...
ENV PATH="/opt/dcm2niix-v1.0.20260724/bin:$PATH"
RUN apt-get update -qq \
    ...
    && cd /tmp/dcm2niix \
    && git fetch --tags \
    && git checkout v1.0.20260724 \
    && mkdir /tmp/dcm2niix/build \
    && cd /tmp/dcm2niix/build \
    && cmake -DZLIB_IMPLEMENTATION=Cloudflare -DUSE_JPEGLS=ON -DUSE_OPENJPEG=ON -DCMAKE_INSTALL_PREFIX:PATH=/opt/dcm2niix-v1.0.20260724 .. \
    && make -j1 \
    && make install \
    && rm -rf /tmp/dcm2niix
RUN apt-get update -qq \
...
RUN printf '{ \
  ...
  "instructions": [ \
    ...
    { \
      "name": "env", \
      "kwds": { \
        "PATH": "/opt/dcm2niix-v1.0.20260724/bin:$PATH" \
      } \
    }, \
    { \
      "name": "run", \
