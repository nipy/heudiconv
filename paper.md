...
![**Weekly downloads experienced an initial sharp rise after the 0.5.1 ReproNim training event in mid 2018, and have subsequently followed a positive trend, whereas usage shows a steady plateau in the data collection interval.** Depicted are monthly download and confirmed session averages, with a 95% confidence interval based on sub-month data. Download counts are sourced from PyPI, the Python community repository; whereas user session counts are sourced from Etelemetry, an infrastructure for verifiable impact information, which end-users can disable to protect privacy.](figs/usage.pdf)
