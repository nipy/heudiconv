...
![**Heudicom conversion and layout can be controlled via custom heuristiscs or by selecting built-in presets.** The heudicom application can be used with the `-c none` parameter to generate a heuristic template for the user to edit, before conversion is performed e.g. via the `-c dcm2niix` parameter which uses the eponymous external application. The process is itempotent, and specifying the `-c dcm2niix` parameter will automatically convert based on a default layout and produce a heuristic directory for the user to inspect post hoc.](figs/workflow.pdf)
