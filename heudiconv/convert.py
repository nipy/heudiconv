            ...
            if outtype == "dicom":
                ...
                if not op.exists(outname) or overwrite:
                    ...
                    res, prov_file = nipype_convert(
                        item_dicoms, prefix, with_prov, bids, tmpdir, dcmconfig
                    )
...
def convert_dicom(item_dicoms, bids, prefix, outdir, tempdirs, symlink, overwrite):
...
def save_converted_files(
    ...
    if isinstance(res_files, list):
        ...
        suffixes = [str(i + 1) for i in range(len(res_files))] if bids else None
        ...
        echo_times = set(load_json(b).get("EchoTime", None) for b in bids_files if b)
