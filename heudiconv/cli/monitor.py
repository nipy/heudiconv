...
def process(paths2process, db, wait=WAIT_TIME, logdir="log"):
    ...
    for path, mod_time in paths2process.items():
        if time.time() - mod_time > wait:
            ...
            process_dict = {"input_path": process_me, "accession_number": op.basename(process_me)}
...
def monitor(topdir="/tmp/new_dir", check_ptrn="/20../../..", db=None, wait=WAIT_TIME, logdir="log"):
    ...
    path_re = re.compile(r"(%s%s)/?$" % (topdir, check_ptrn))
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        prog="monitor.py",
        description=("Small monitoring script to detect new directories and " "process them"),
...
def main():
    ...
    monitor(parsed.path, parsed.check_ptrn, db, wait=parsed.wait_time, logdir=parsed.logdir)
